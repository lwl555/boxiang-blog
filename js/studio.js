...
(function () {
  ...
  const SYSTEM_PROMPT = '你是一位顶级网页设计师与前端工程师，为「薄想工作室」的免费 AI 建站工作台服务。\n' +
    ...
    '【阶段3 生成网站】用户确认文档（回复「可以」「没问题」「开始」「就这样」等）后，才输出一个完整、可直接运行的 HTML 文档（以 <!DOCTYPE html> 开头，以 </html> 结尾）。输出的是纯 HTML 文本：禁止使用任何 Markdown 代码围栏（\`\`\` 或 \`\`\`html），禁止在 HTML 前后添加任何解释文字、注释说明或问候语；</html> 必须是输出的最后内容。\n' +
    ...
    '20. 【账号系统·新能力】本平台支持给网站接入注册/登录（会员）系统，发布时系统会自动注入账号逻辑，**不要自己编写登录相关的 JS**：注册表单写 <form data-bx-auth="register">，内部必须包含三个输入框 name="username"（用户名）、name="password"（密码，至少 6 位）、name="confirm"（确认密码），再加一个提交按钮；登录表单写 <form data-bx-auth="login">，含 name="username"、name="password"；退出按钮写 <button data-bx-auth="logout">退出登录</button>；会员专属内容（登录后才能看的部分）包在 <div data-bx-auth-only>...</div> 里；登录/注册表单区域包在 <div data-bx-guest-only>（登录后自动隐藏）；需要显示当前用户名的地方放 <span data-bx-user></span>。每个网站账号上限 50 个。\n' +
    '21. 【深度理解用户】动笔前必须做三件事：① 判断网站类型（餐饮/作品集/电商/教育/本地服务/个人品牌/活动/工具等）；② 推断这个行业网站该有的板块并合理取舍；③ 站在访客角度想：点进来最想看什么、怎么联系或下单。举例：咖啡店网站要有菜单、营业时间、地址导航、氛围图；设计师作品集要有案例图、服务报价、联系入口；小店要有商品、价格、购买方式。用户没说但明显必要的内容（营业时间、价格、联系方式、地址）要主动补上，用占位文字代替，禁止留空。\n' +
    '22. 【颜值标准·按这8条自检】页面必须达到：① 首屏 Hero 有大字标题（40px 以上）、一句副标题、一个明显行动按钮，背景有细腻渐变或光晕；② 板块之间有充足留白（上下间距 60px 以上），每个板块有清晰小标题；③ 卡片圆角 14~20px、柔和阴影、hover 时轻微上浮；④ 按钮有 hover 颜色过渡和点击按压反馈；⑤ 导航吸顶、半透明毛玻璃效果；⑥ 全页有 2~3 种强调色点缀（主色+辅助色），禁止单色到底；⑦ 文字排版讲究：标题粗而有力量，正文 15~17px、行高 1.7 左右，段落间距舒适；⑧ 重要内容用滚动显现动画（进入视口淡入上移）增强质感。输出前逐条对照，做不到就重写。\n';
  ...
  function renderSessions() {
    ...
    list.innerHTML = sessions.slice().sort((a, b) => ((b.published ? 1 : 0) - (a.published ? 1 : 0))).map((s) => {
      ...
      return '<div class="session-item' + (active ? ' active' : '') + '" data-sid="' + s.id + '">' +
        ...
        (s.apis && s.apis.length ? ' · 🧩' + s.apis.length + '个API' : '') + (s.backend ? ' · 🔧 简易后台' : '') + (s.auth ? ' · 🔐 账号系统' : '') +
        '</span>' +
  ...
  function extractHtml(text) {
    ...
    // 移除所有独立成行的 Markdown 代码围栏（``` / ```html 等），防止残留到网页里
    t = t.replace(/^```[a-zA-Z]*\s*$/gm, '');
    // 若仍有围栏（不成行的情况），从最后一个围栏截断
    const li = t.lastIndexOf('```');
    if (li >= 0) t = t.slice(0, li).trim();
    // 只要最后一个 </html> 之前的代码，后面的解释文字/围栏全部丢弃
    const he = t.toLowerCase().lastIndexOf('</html>');
    if (he >= 0) t = t.slice(0, he + 7).trim();
  ...
  function preview(html) {
    ...
    const slug = backend && backend.slug ? backend.slug : '';
    let out = slug
      ? injectBackend(lastHtml, slug, backend.token, false)
      : injectBackend(lastHtml, '', '', true);
    out = injectAuth(out, slug, !slug);
    $('stFrame').srcdoc = out;
  }
  ...
  async function showSiteData(s) {
    ...
    try {
      ...
      let out = '';
      if (!rows.length) {
        out = '<div class="empty-tip">📭 还没有收到任何提交。去网站上试试表单吧。</div>' +
          '<p class="data-hint">🔧 每个网站后端上限 100 条；站长在上线后也可通过网站自带的「数据管理页」查看。</p>';
      } else {
        out = '<p class="data-count">✅ 共 ' + rows.length + ' 条提交（最新在前）</p>' +
      ...
      if (s.auth) {
        try {
          const { data: users, error: uerr } = await sb.rpc('site_user_list', { p_slug: s.backend.slug, p_token: s.backend.token });
          if (!uerr && users && users.length) {
            out += '<p class="data-count" style="margin-top:16px">🔐 已注册账号：' + users.length + ' 个（上限 ' + AUTH_MAX_USERS + '）</p>' +
              '<div class="data-table-wrap"><table class="data-table"><thead><tr><th>用户名</th><th>注册时间</th></tr></thead><tbody>' +
              users.map((d) => '<tr><td>' + esc(d.username || '-') + '</td><td class="time">' + fmtDT(d.created_at) + '</td></tr>').join('') +
              '</tbody></table></div>';
          }
        } catch (e) { /* 用户列表失败不影响表单数据 */ }
      }
      body.innerHTML = out;
    } catch (e) {
  ...
  function authScriptHtml(slug, preview) {
    const cfg = { slug: slug || '', preview: !!preview, url: window.SUPABASE_URL + '/rest/v1/rpc', key: window.SUPABASE_ANON_KEY };
    return '<script>window.BX_AUTH=' + JSON.stringify(cfg) + ';(function(){var C=window.BX_AUTH;' +
      'function ready(fn){if(document.readyState!=="loading"){fn();}else{document.addEventListener("DOMContentLoaded",fn);}}' +
      'function errMsg(t){try{var j=JSON.parse(t);return j.message||t;}catch(e){return t;}}' +
      'function call(name,body){return fetch(C.url+"/"+name,{method:"POST",headers:{"Content-Type":"application/json","apikey":C.key,"Authorization":"Bearer "+C.key},body:JSON.stringify(body)}).then(function(r){return r.text().then(function(t){if(!r.ok){throw new Error(errMsg(t));}try{return JSON.parse(t);}catch(e){return null;}});});}' +
      'var TOK_KEY="bx_auth_"+C.slug;' +
      'function apply(){var on=!!C.user;document.querySelectorAll("[data-bx-auth-only]").forEach(function(el){el.style.display=on?"":"none";});document.querySelectorAll("[data-bx-guest-only]").forEach(function(el){el.style.display=on?"none":"";});document.querySelectorAll("[data-bx-user]").forEach(function(el){el.textContent=C.user||"";});}' +
      'function setUser(u,tok){C.user=u||null;if(tok){try{localStorage.setItem(TOK_KEY,tok);}catch(e){}}if(!u){try{localStorage.removeItem(TOK_KEY);}catch(e){}}apply();}' +
      'function doLogout(){if(C.preview){alert("预览模式：发布上线后即可登录/注册");return;}var tok="";try{tok=localStorage.getItem(TOK_KEY)||"";}catch(e){}if(tok){try{call("site_logout",{p_slug:C.slug,p_token:tok});}catch(e){}}setUser(null);}' +
      'ready(function(){var tok="";try{tok=localStorage.getItem(TOK_KEY)||"";}catch(e){}if(tok&&!C.preview){call("site_me",{p_slug:C.slug,p_token:tok}).then(function(u){setUser(u);}).catch(function(){setUser(null);});}else{apply();}' +
      'document.querySelectorAll("form[data-bx-auth=register]").forEach(function(f){f.addEventListener("submit",function(ev){ev.preventDefault();if(C.preview){alert("预览模式：发布上线后即可注册");return;}var d=new FormData(f);var u=String(d.get("username")||"").trim();var p=String(d.get("password")||"");var c=String(d.get("confirm")||"");if(u.length<2||u.length>20){alert("用户名需 2~20 个字符");return;}if(p.length<6){alert("密码至少 6 位");return;}if(c&&p!==c){alert("两次输入的密码不一致");return;}var b=f.querySelector("[type=submit],button");if(b){b.disabled=true;b.textContent="注册中…";}call("site_register",{p_slug:C.slug,p_username:u,p_password:p}).then(function(){return call("site_login",{p_slug:C.slug,p_username:u,p_password:p});}).then(function(tok){setUser(u,tok);if(b){b.disabled=false;b.textContent="注册";}alert("🎉 注册成功，已自动登录！");f.reset();}).catch(function(e){if(b){b.disabled=false;b.textContent="注册";}alert("⚠️ 注册失败："+e.message);});});});' +
      'document.querySelectorAll("form[data-bx-auth=login]").forEach(function(f){f.addEventListener("submit",function(ev){ev.preventDefault();if(C.preview){alert("预览模式：发布上线后即可登录");return;}var d=new FormData(f);var u=String(d.get("username")||"").trim();var p=String(d.get("password")||"");var b=f.querySelector("[type=submit],button");if(b){b.disabled=true;b.textContent="登录中…";}call("site_login",{p_slug:C.slug,p_username:u,p_password:p}).then(function(tok){setUser(u,tok);if(b){b.disabled=false;b.textContent="登录";}alert("✅ 登录成功，欢迎回来 "+u+"！");f.reset();}).catch(function(e){if(b){b.disabled=false;b.textContent="登录";}alert("⚠️ 登录失败："+e.message);});});});' +
      'document.querySelectorAll("[data-bx-auth=logout]").forEach(function(el){el.addEventListener("click",function(ev){ev.preventDefault();doLogout();});});' +
      '})();</script>';
  }
  function injectAuth(html, slug, preview) {
    if (!/data-bx-auth|data-bx-user|data-bx-auth-only|data-bx-guest-only/i.test(html) || !/<\/body>/i.test(html)) return html;
    return html.replace(/<\/body>/i, authScriptHtml(slug, preview) + '\n</body>');
  }
  ...
  async function publish() {
    ...
    try {
      ...
      const url = BASE + '/sites/' + slug + '/';
      const baseHtml = ensureFooter(lastHtml);
      const hasForm = /<form[\s>]/i.test(baseHtml);
      const hasAuth = /data-bx-auth|data-bx-user|data-bx-auth-only|data-bx-guest-only/i.test(baseHtml);
      const token = (hasForm || hasAuth) ? 't_' + Math.random().toString(36).slice(2, 10) + Date.now().toString(36) : '';
      let siteHtml = baseHtml;
      let backendTip = '';
      let authTip = '';
      if (hasAuth) {
        try {
          siteHtml = injectAuth(siteHtml, slug, false);
          await sb.rpc('setup_site_auth', { p_slug: slug, p_token: token, p_content_html: siteHtml, p_client_id: getClientId(), p_max_users: AUTH_MAX_USERS });
          if (current) {
            current.auth = true;
          ...
          authTip = '🔐 你的网站已接入<b>账号系统</b>：访客可注册、登录、退出，会员专属内容自动隐藏/显示（每站上限 ' + AUTH_MAX_USERS + ' 个账号）。点「我的网站 → 📊 数据」可查看注册用户。';
        } catch (be) {
          authTip = '⚠️ 网站已发布，但账号系统接入失败：' + esc(be.message || '') + '。请在 Supabase → SQL Editor 运行 upgrade-v8.sql 升级脚本后，重新发布一次。';
        }
      }
      if (hasForm) {
        try {
          siteHtml = injectBackend(siteHtml, slug, token, false);
          ...
          backendTip = '🔧 你的网站已接入<b>简易后台</b>：访客提交的表单数据会自动保存（上限 ' + BACKEND_MAX_ROWS + ' 条）。点「我的网站 → 📊 数据」即可查看。';
        } catch (be) {
